...
<table><tr><td>
<b>Note:</b> We might want to consider multiplying the number of risk alleles by some
factor (e.g. 0.5) to correct for possible overestimation of the effect sizes
due to publication bias. In other words, consider running <code>geno <- 0.5 * geno</code>.
</td></tr></table>
...
We choose <i>α<sub>k</sub></i> for each disease *k* to ensure that the mean
probability <i>P&#773;<sub>k</sub></i> is equal to the known prevalence
<i>V<sub>k</sub></i> of the disease in the population.

We do this by minimizing <i>P&#773;<sub>k</sub> - V<sub>k</sub></i> with golden
